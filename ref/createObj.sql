...
ALTER FUNCTION [dbo].[getCoursePaperID](@ID int, @refID int)
RETURNS varchar(4000)
AS
BEGIN
	declare @re varchar(4000), @p int, @item nvarchar(50),@score decimal(18, 2)
	select @re = ''
	--select @re = paperID from studentExamList where refID=(case when @ID>0 then @ID else @refID end) 
	declare rc cursor for select a.paperID,isnull(b.memo,'') item,isnull(a.score,0) from studentExamList a, examInfo b where a.examID=b.examID and a.kind=0 and a.refID=(case when @ID>0 then @ID else @refID end)
	open rc
	fetch next from rc into @p,@item,@score
	While @@fetch_status=0 
	Begin 
		select @re = @re + '{"paperID":' + cast(@p as varchar) + ',"item":"' + @item + '","examScore":' + cast(@score as varchar) + '},'
		fetch next from rc into @p,@item,@score
	End
	Close rc 
	Deallocate rc
...
BEGIN
	...
	begin
		...
		if @re = 0
		begin
			insert into studentInfo(host,userName,name,password,kindID,companyID,dept1,dept2,dept3,job,job_status,mobile,phone,email,address,education,unit,dept,ethnicity,IDaddress,bureau,IDdateStart,IDdateEnd,experience,memo,birthday,sex,age,linker,fromID,registerID) 
				values(@host,upper(@username),@name,@password,@kindID,@companyID,@dept1,@dept2,@dept3,@job,@job_status,@mobile,@phone,@email,@address,@education,@unit,@dept,@ethnicity,@IDaddress,@bureau,@IDdateStart,@IDdateEnd,@experience,@memo,substring(@username,7,8),dbo.getSexfromID(@username),dbo.getAgefromID(@username),@linker,@fromID,@registerID)
...
BEGIN
	...
	if exists(select 1 from studentInfo where username=@username)
		...
		if @re = 0
		begin
			insert into studentInfo(host,userName,name,kindID,companyID,dept1,dept2,dept3,job,job_status,mobile,phone,email,address,education,unit,dept,ethnicity,IDaddress,bureau,IDdateStart,IDdateEnd,experience,memo,birthday,sex,age,linker,fromID,registerID) 
				values(@host,upper(@username),@name,@kindID,@companyID,@dept1,@dept2,@dept3,@job,@job_status,@mobile,@phone,@email,@address,@education,@unit,@dept,@ethnicity,@IDaddress,@bureau,@IDdateStart,@IDdateEnd,@experience,@memo,@birthday,@sex,@age,@linker,@fromID,@registerID)
			select @userID=userID from studentInfo where username=@username
			select @event = '新增'
			exec writeEventTrace @host,@registerID,'user',@username,0,'登记',@username
		end
	...
	begin
		...
		begin
			...
			begin
				...
				--添加课程
				insert into studentCourseList(username,courseID,refID,payNow,title,price,type,hours,closeDate,projectID,classID,SNo,reexamine,checked,checkDate,checker,currDiplomaID,currDiplomaDate,fromID,host,registerID) select @username,courseID,@refID,@payNow,@title,@price,@type,hours,dateadd(d,period,getDate()),@projectID,@classID,@SNo,@reexamine,@checked,@checkDate,@checker,@currDiplomaID,@currDiplomaDate,@fromID,@host,@username from courseInfo where courseID=@courseID
...
AS
BEGIN
	update studentInfo set password='123456' where username=@username
	-- 写操作日志
	exec writeOpLog '','重置密码','resetStudentPwd',@registerID,'',@username
END
...
AS
BEGIN
	declare @re int, @passwd varchar(50)
	select @re = 0, @passwd = item from dictionaryDoc where kind='studentPasswd' and ID='0'
	if not exists(select 1 from studentInfo where username=@username)
		set @re = 1
	else
		if not exists(select 1 from studentInfo where username=@username and mobile=@phone)
			...
			update studentInfo set password=@passwd where username=@username
	
	select @re as status, @passwd as password
END
...
BEGIN
	...
	declare @retireDay int, @hasLesson int

	if @classID='' or @classID='null' set @classID=null
	if @currDiplomaDate='' or @currDiplomaDate='null' set @currDiplomaDate=null
	if @currDiplomaID='' or @currDiplomaID='null' set @currDiplomaID=null
	select @re=0,@msg=iif(@ID=0,'报名成功。','保存成功。'),@hasLesson=0, @datePay=[dbo].[whenull](@datePay,null), @username=upper(@username), @host=[dbo].[whenull](@host,'')
	...
	begin
		...
		select @oldClass=classID, @pNo=SNo, @autoPay=autoPay,@courseID=courseID from studentCourseList where ID=@ID
...
ALTER PROCEDURE [dbo].[updateGeneratePasscardInfo1]
	@ID int,@certID varchar(50),@title nvarchar(100),@startNo int,@kindID int,@startDate varchar(100),@startTime varchar(100),@address nvarchar(100),@notes nvarchar(500),@sync int,@memo nvarchar(500),@registerID varchar(50)
AS
BEGIN
	...
	begin
		--填写生成记录
		insert into generatePasscardInfo(certID,title,startNo,kindID,startDate,startTime,address,notes,sync,memo,registerID) values(@certID,@title,@startNo,@kindID,@startDate,@startTime,@address,@notes,@sync,@memo,@registerID)
		select @re=max(ID),@event='添加考试' from generatePasscardInfo where registerID=@registerID
	end
	if @ID>0	-- 保存信息
	begin
		update generatePasscardInfo set title=@title,kindID=@kindID,startDate=@startDate,startTime=@startTime,address=@address,notes=@notes,sync=@sync,memo=@memo where ID=@ID
		select @event='修改考试'
	end
	-- 写操作日志
...
AS
BEGIN
	declare @startDate smalldatetime, @courseID varchar(50), @online int, @n int,@seq int,@kindID int,@typeID int,@hours int,@period varchar(50),@item nvarchar(100), @theDate smalldatetime,@teacher varchar(50),@classroom varchar(50),@re int
	if @mark='A'
	...
	begin
		...
		declare rc cursor for select seq,kindID,typeID,online,hours,period,item from schedule where courseID=@courseID and status=0 order by seq
		open rc
		fetch next from rc into @seq,@kindID,@typeID,@online,@hours,@period,@item
		While @@fetch_status=0 
		Begin
			--计算当前日期
			...
			insert into classSchedule(mark,classID,courseID,seq,kindID,typeID,online,hours,period,theDate,theWeek,item,address,teacher,registerID)
				select @mark,@classID,@courseID,@seq,@kindID,@typeID,@online,@hours,@period,@theDate,datepart(weekday,@theDate),@item,@classroom,@teacher,@registerID
			if @typeID=1
...
ALTER PROCEDURE [dbo].[updateClassSchedule]
	@ID int,@seq int,@kindID int,@typeID int,@online int,@hours int,@period varchar(50),@theDate varchar(50),@teacher varchar(50),@address nvarchar(100),@item nvarchar(100),@memo varchar(500),@registerID varchar(50)
AS
BEGIN
	...
	--else
		update classSchedule set seq=@seq,kindID=@kindID,typeID=@typeID,online=@online,item=@item,hours=@hours,period=@period,theDate=@theDate,theWeek=datepart(weekday,@theDate),teacher=@teacher,address=@address,memo=@memo,registerID=@registerID where ID=@ID
...
ALTER PROCEDURE [dbo].[updateStandardSchedule]
	@ID int, @courseID varchar(50),@seq int,@kindID int,@typeID int,@online int,@hours int,@period varchar(50),@item nvarchar(100),@memo nvarchar(500),@registerID varchar(50)
...
BEGIN
	...
	if @ID=0
	begin
		insert into schedule(courseID,seq,kindID,typeID,online,hours,period,item,memo,registerID) values(@courseID,@seq,@kindID,@typeID,@online,@hours,@period,@item,@memo,@registerID)
		select @ID=max(ID) from schedule where courseID=@courseID
	end
	else
		update schedule set seq=@seq,kindID=@kindID,typeID=@typeID,online=@online,hours=@hours,period=@period,item=@item,memo=@memo,registerID=@registerID where ID=@ID
...
-- =============================================
ALTER PROCEDURE syncPaper4Exam 
	@batchID int
AS
BEGIN
	-- SET NOCOUNT ON added to prevent extra result sets from
...
-- 获取安监班级的刷脸考勤表
-- USE CASE: exec [getClassCheckinList] '308'
...
BEGIN
	...
	SET @SqlStatement = N'
	declare @tb table(passNo varchar(50),ID int,enterID int, qty int, username varchar(50), name nvarchar(50)' + @fields + ')
	declare @mark int
	...
	insert into @tb
		SELECT * FROM (
		  SELECT
			...
			theDate,
			d.refID
		  FROM (select * from (select passNo,ID as applyID,enterID, username, name from v_applyInfo where refID=' + @classID + ') a,
...
--mark: D 日报  M 月报
CREATE PROCEDURE [dbo].[getIncomeRpt]
...
BEGIN
	...
	create table #tbl (datePay varchar(50), p1 int, p2 int, p3 int, p4 int, p5 int, p6 int, p7 int, p int)
	-- p7退款 p付款小计
	select @where = ''
	if @startDate > ''
	...
	if @sales > ''
		select @where=@where + ' and sales=''' + @sales + ''''
	if @where > ''
		select @where = @where + ' and host=''' + @host + '''' + ' and partnerID=0'

	select @sql=',sum(iif(pay_type=1,amount,0)) as p1,sum(iif(pay_type=2,amount,0)) as p2,sum(iif(pay_type=4,amount,0)) as p3,sum(iif(pay_type=5,amount,0)) as p4,sum(iif(pay_type=0,amount,0)) as p5,sum(iif(pay_type=3,amount,0)) as p6, 0 as p7 from v_studentCourseList where ' + @where + ' group by '
	if @mark='D'
	...
	if @sales > ''
		select @where1=@where1 + ' and sales=''' + @sales + ''''
	if @where1 > ''
		select @where1 = @where1 + ' and host=''' + @host + '''' + ' and partnerID=0'
	if @mark='D'
		select @sql= 'insert into #tbl select datePay, sum(isnull(p1,0)), sum(isnull(p2,0)), sum(isnull(p3,0)), sum(isnull(p4,0)), sum(isnull(p5,0)), sum(isnull(p6,0)), sum(isnull(p7,0)), 0 from (' + @sql + ' union all select dateRefund,0,0,0,0,0,0,sum(refund_amount) from v_studentCourseList where ' + @where1 + ' group by dateRefund) a group by datePay'
	if @mark='M'
		select @sql= 'insert into #tbl select datePay as datePay, sum(isnull(p1,0)), sum(isnull(p2,0)), sum(isnull(p3,0)), sum(isnull(p4,0)), sum(isnull(p5,0)), sum(isnull(p6,0)), sum(isnull(p7,0)), 0 from (' + @sql + ' union all select left(dateRefund,7),0,0,0,0,0,0,sum(refund_amount) from v_studentCourseList where ' + @where1 + ' group by left(dateRefund,7)) a group by datePay'

	EXECUTE (@sql)

	update #tbl set p = p1+p2+p3+p4+p5+p6
	insert into #tbl select '合计',sum(p1),sum(p2),sum(p3),sum(p4),sum(p5),sum(p6),sum(p7),sum(p) from #tbl
	select datePay as 日期, p1 as 财付通G, p2 as 财付通S, p3 as 微信收款, p4 as 支付宝收款, p5 as 现金收款, p6 as 对公汇款, p as 收款小计, p7 as 退款, p-p7 as 合计 from #tbl where datePay>'' order by datePay	-- pp 合计
END
GO
...
BEGIN
	--将名单导入到临时表
	create table #temp_receive_check(id varchar(50))
	declare @n int, @j int
	select @n=dbo.pf_getStrArrayLength(@selList,','), @j=0
	while @n>@j
	begin
...
ALTER PROCEDURE [dbo].[getPayInvoiceRpt]
	@startDate varchar(50), @endDate varchar(50), @startDate1 varchar(50), @endDate1 varchar(50), @host varchar(50), @autoPay int, @autoInvoice int, @receivable int
AS
BEGIN
...
AS
BEGIN
	declare @tb table(enterID int,kindID int,mark nvarchar(50) default(''),autoPay int,autoInvoice int,username varchar(50) default(''), name nvarchar(50) default(''), price int, amount int, datePay varchar(50) default(''), pay_type int, pay_typeName nvarchar(50) default(''), shortName nvarchar(50) default(''),noReceive int,invoice varchar(50) default(''),dateInvoice varchar(50) default(''), title nvarchar(200) default(''), autoPayName nvarchar(50), autoInvoiceName nvarchar(50), pay_memo nvarchar(500) default(''),invoicePDF varchar(2000) default(''))
	
	--当天收费记录
	insert into @tb select ID,0,'',autoPay,autoInvoice,username, name, price, amount, datePay, pay_type, pay_typeName, shortName,noReceive,invoice,dateInvoice, dbo.getInvoiceTitle(title), '', '',pay_memo,[dbo].[getCourseInvoice](ID) as invoicePDF from v_studentCourseList where datePay=@startDate and amount>0 and pay_status>0 and host in('znxf','spc','shm')
	--当天退款记录
	...
	if @mark='file'
		select iif(kindID<5,cast(ROW_NUMBER() OVER (ORDER BY kindID,enterID) as varchar),'') as 'No',name as '姓名',datePay as '付款日期',autoPayName as '付款类型',dateInvoice as '开票日期',autoInvoiceName as '开票类型',invoice as '数电票号',title as '开票抬头',shortName as '项目',pay_typeName as '付款方式',price as '单价',iif(kindID<4,1,null) as '人数',amount as '金额',mark as '标识',pay_memo as '备注' from @tb order by kindID,enterID,pay_type
END
GO
...
AS
BEGIN
	select a.ID, a.theDate, a.typeID, item, isnull(a.teacherName,'未知') as teacherName from v_classSchedule a, generateApplyInfo b where a.classID=b.ID and a.mark='A' and a.status=0 and b.applyID=@applyID order by a.seq
END
GO
