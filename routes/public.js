...
//4. 批量通知学员，考试时间地点。同时发送系统消息和短信。
router.post('/send_message_exam_apply', function(req, res, next) {
...
//4. 批量通知学员，复训日期将到期。发送短信。
router.post('/send_message_review_alert', function(req, res, next) {
  sqlstr = "sendMsg4Review";
  params = {mark:req.body.batchID, registerID: req.body.registerID };
  db.excuteProc(sqlstr, params, function (err, data) {
    if (err) {
      console.log(err);
      let response = { "status": 9, msg:"系统错误。" };
    ...
    if(req.body.SMS==1){ //发通知
      for (var i in re){
        if(re[i]["mobile"].length == 11){
          ...
          sqlstr = "writeSSMSlog";
          params = { username: re[i]["username"], mobile: re[i]["mobile"], kind: "复训通知", message: re[i]["item"], refID: re[i]["username"], registerID: req.body.registerID };
          //console.log(params);
          db.excuteProc(sqlstr, params, function (err, data1) {
            if (err) {
              console.log(err);
...
//4. 批量通知学员，考试成绩。同时发送系统消息和短信。
router.post('/send_message_score_apply', function(req, res, next) {
    let ec = 0;
...
//
router.post('/marketScoreCheck', function (req, res) {
  // 配置初始化信息 selList: when kindID:0 applyID  1 enterID  2 username
  shell.exec('@echo off')
  // shell.exec('chcp 65001')
  let url = pyUrl + '/marketScoreCheck.py ' + req.body.selList + ' ' + req.body.kindID + ' ' + req.body.refID + ' ' + req.query.host + ' ' + req.query.register;
  // console.log("url:", url)
  shell.exec(url, function (code, stdout, stderr) {
    // console.log('Exit code:', code);
    // console.log('Program output:', stdout);    
...
//4. 将一批学员的发票设为一组
router.post('/setInvoiceGroupCancel', function(req, res) {
  sqlstr = "setInvoiceGroupCancel";
  params = { selList:req.body.selList, kind:req.body.kind, classID:req.body.classID, registerID:req.body.registerID };
  // console.log('setInvoiceGroup',params);
  db.excuteProc(sqlstr, params, function (err, data) {
    if (err) {
      console.log(err);
...
//11a. getAgreementList 获取某个学员的协议列表
router.get('/getAgreementList', function (req, res, next) {
  //firstly check the paper has its questions, if has not, create them now.
  params = { username: req.query.username };
  sqlstr = "getStudentAgreementList";
  db.excuteProc(sqlstr, params, function (err, data) {
    if (err) {
      console.log(err);
      let response = { "status": 9 };
