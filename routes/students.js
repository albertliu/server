...
router.get('/getStudentCourseList', function (req, res, next) {
  ...
  if(req.session.user.teacher > ""){  //教师按照自己的课程给出列表
    ...
    sqlstr = "select ID,username,classID,file4,courseName,reexamineName,reexamine,type,checkName,status,statusName,completion,completion1,hours,startDate,endDate,pre,signatureType,signature,agencyID,host,regDate,payNow,pay_status,invoice,autoPay,examScore,examTimes,[dbo].[getPassCondition](ID) as pass_condition,dbo.getCoursePaperID(ID, refID) AS paperID,dbo.getCourseHelps(ID) AS helps from v_studentCourseList where username='" + req.query.username + "' and status<2" + " order by status";
    params = {};
  }
